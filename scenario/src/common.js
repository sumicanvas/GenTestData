...
const DEFAULT_COLLECTION = "news_mig";
const DEFAULT_INDEX = "news_search_index";
...
async function appendLog(scenario, message) {
  ...
  await appendFile(join(LOG_DIR, `${safeLogName(scenario)}.log`), `${message}\n`, "utf-8");
}
...
function toInt(value, fallback) {
  ...
  if (Number.isNaN(parsed) || parsed <= 0) throw new Error(`Invalid positive integer: ${value}`);
  return parsed;
}
...
async function promptMissingValues(args, inputs) {
  ...
  if (missing.length === 0) return Object.fromEntries(inputs.map((item) => [item.key, args[item.key]]));

  if (!input.isTTY) {
    throw new Error(`Missing required arguments: ${missing.map((item) => `--${item.key}`).join(", ")}`);
  }
  ...
  try {
    for (const item of missing) {
      ...
      if (!answer.trim()) throw new Error(`Missing value for ${item.key}`);
...
export function projectStage({ score = false, highlights = false } = {}) {
  ...
  if (score) projection.score = { $meta: "searchScore" };
  if (highlights) projection.highlights = { $meta: "searchHighlights" };
...
export function textMust(query, path = "title") {
  return {
    text: {
      query,
      path,
...
export function keywordPipeline({ indexName, limit, query, filters = [], highlight = false, searchPath = "title", highlightPath = searchPath }) {
  const compound = { must: [textMust(query, searchPath)] };
  ...
  if (highlight) search.highlight = { path: highlightPath };
...
export function fuzzyPipeline({ indexName, limit, query, titleBoost = 5, contentsBoost = 1, minScore = 1 }) {
  const fuzzy = { maxEdits: 1, prefixLength: 1, maxExpansions: 50 };
  const pipeline = [
    {
      $search: {
        index: indexName,
  ...
  if (minScore > 0) pipeline.push({ $match: { score: { $gte: minScore } } });
...
export async function runScenario({ scenario, description, inputs = [], buildPipeline }) {
  ...
  if (config.dryRun) {
    ...
    if (loggingEnabled) await printAndLog(scenario, dryRunMessage);
    else console.error(dryRunMessage);
    console.log(formatMql(config.collectionName, pipeline));
    return;
  }

  if (!config.uri) throw new Error("MONGODB_URI 환경변수를 설정하거나 --uri 옵션을 전달하세요.");
  ...
  try {
    ...
    if (loggingEnabled) await printAndLog(scenario, mqlMessage);
    else console.error(mqlMessage);
    ...
    if (loggingEnabled) await printAndLog(scenario, startMessage);
    else console.error(startMessage);

    const documents = await client.db(config.dbName).collection(config.collectionName).aggregate(pipeline).toArray();
    ...
    if (loggingEnabled) await printAndLog(scenario, endMessage);
    else console.error(endMessage);

    console.log(JSON.stringify(documents, null, 2));

    if (loggingEnabled) await printAndLog(scenario, durationMessage);
    else console.error(durationMessage);
  } catch (error) {
    const failMessage = `쿼리실패: ${nowIso()} error=${error.message}`;
    if (loggingEnabled) await printAndLog(scenario, failMessage);
    else console.error(failMessage);
    throw error;
  } finally {
    await client.close();
  }
