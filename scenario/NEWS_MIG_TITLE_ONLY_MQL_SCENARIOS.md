...
```sh
npm run mig:4 -- --shcode 005930 --dgubun 4 --query "삼성전자" --limit 5 --dry-run
```

실제 실행은 `--dry-run`을 제거한다.

```sh
npm run mig:4 -- --shcode 005930 --dgubun 4 --query "삼성전자" --limit 5
```
...
npm run mig:4 -- --shcode 005930 --dgubun 4 --query "삼성전자" --dry-run --limit 1 --no-log
