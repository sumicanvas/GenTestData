# news_mig_500 시나리오별 Node.js 실행 프로그램

`news_mig_500` 컬렉션을 대상으로 Atlas Search MQL을 시나리오별 Node.js 프로그램으로 실행하기 위한 스크립트다.
...
`news_mig_500`은 migration 결과 컬렉션이므로 종목코드가 아래처럼 object 배열에 들어가는 구조를 기본값으로 둔다.
...
따라서 종목코드 필터 기본 path는 다음이다.

```text
shcode.shcode
```

만약 target 컬렉션이 `shcode: ["005930"]` 구조라면 실행 시 아래 옵션을 사용한다.
...
```sh
node news_mig_500_scenarios/scenario01_all_news.js --limit 5
```
...
```sh
npm run mig500:3 -- --shcode 005930 --query "삼성전자 실적" --limit 5 --dry-run
```
...
| `--index` | Atlas Search index 이름 | `news5_search_index` |
| `--shcode-path` | 종목코드 Atlas Search path | `shcode.shcode` |
| `--limit` | 조회 건수 | `100` |
...
실행 MQL, 쿼리 시작/종료 시각, 수행시간은 stderr와 아래 로그 파일에 출력된다.

```text
news_mig_500_scenarios/logs/
```

조회 결과 JSON은 stdout으로 출력된다.
