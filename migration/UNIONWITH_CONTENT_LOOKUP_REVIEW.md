...
고객 데이터에서 `SEQNO`가 문자열로 잘 유지된다면 아래 비교로 충분하다.

```js
{ $eq: [{ $toString: "$SEQNO" }, { $toString: "$$seq" }] }
```

하지만 import 과정에서 `SEQNO`가 숫자로 들어가 앞자리 0이 사라졌다면 아래처럼 8자리 padding 비교가 필요하다.

```text
"08000721" vs 8000721
```

이 경우 `$toString`만 쓰면 매칭되지 않는다.
      ...
      pipeline: [
        {
          $match: {
            $expr: {
              $and: [
                { $eq: [{ $toString: "$YMD" }, { $toString: "$$ymd" }] },
                { $eq: [{ $toString: "$NEWSCODE" }, { $toString: "$$newscode" }] }
              ]
            }
          }
        },
