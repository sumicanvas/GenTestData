...
# 오라클 JSON_OBJECT() 로 export 한 데이터가 value 를 "" 으로 감싸고 있지않아 문제가 발생,
# 데이터파일(파일 수정) 후처리를 수행할 파이썬 프로그램 정의, mast와 jmcode는 단건들이라 따로 처리
# 뉴스 내용 부분에 해당하는 전체 데이터파일을 후처리하는 프로그램
...
function efixer {
    ...
    # 예) python repair_news_mast_json.py --input input/news_mast.json
    CMD="python3.9 ${EFIXER} --input ../input/${IFILE}"
    
    # 실행할 명령어를 화면에 출력합니다.
    echo $CMD
    
    # 실제로 파이썬 스크립트를 실행하여 데이터를 온전한 JSON으로 변환
    python3.9 ${EFIXER} --input ../input/${IFILE}
}

# 2. 복원된 JSON 파일을 몽고디비에 임포트하는 함수입니다. 
# 첫번째 인자: 파일명(output/<변환된 데이터>.json, 두번째 인자 : 콜렉션명
# 예) import2db "${filenoext}.json" "${collname}"
function import2db {
